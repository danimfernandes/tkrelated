...
freqCommonSNPsAnyDataset = function(spagedi.freq.file,dataset) {
...
freqCommonSNPsSameFreqFile = function(spagedi.freq.file,plink.freq.file) {
  ids=strsplit(spagedi.freq.file,"_spagedi_frequencies.txt")
  freqsFile=read.table(paste0(spagedi.freq.file),as.is=TRUE,header=FALSE,stringsAsFactors = FALSE,sep="\t",nrows=1)
  to_rem=seq(from=2,to=length(freqsFile),by=2)
  freqsFile=freqsFile[,-to_rem]
  freqsFileT=t(freqsFile)
  write.table(freqsFileT,paste0("common_alleles_",ids),quote = FALSE,sep="\t", row.names = FALSE, col.names = FALSE)
  alFreq = read.csv(plink.freq.file,stringsAsFactors=FALSE,sep="",colClasses = c(character(),character(),character(),character(),numeric(),numeric()))
  common_freqs_plink_format = data.frame(CHR=as.numeric(),SNP=as.numeric(),A1=as.character(),A2=as.character(),MAF=as.character(),NCHROBS=as.character())
  for(i in freqsFileT) {
    if(i %in% alFreq$SNP) {
      posInAlFreq=(which(alFreq$SNP == i))
      print(posInAlFreq)
      newRow=alFreq[posInAlFreq,]
      common_freqs_plink_format=rbind(common_freqs_plink_format,newRow)
    }
  }
  write.table(format(common_freqs_plink_format,digits=6),paste0("commons_freqs_",ids,".frq"), quote=FALSE,row.names = FALSE,col.names = FALSE,sep="\t")  
}
